/**
 * Scheduled KPI Report — runs on the 1st of each month at 8am NZST
 * Cron: "0 0 8 1 * *" (NCRONTAB format used by Azure Functions)
 *
 * Determines which report frequencies to send based on the current date:
 *  - Monthly:   every run (1st of month)
...
// ─── KPI metric computation (mirrors client-side computeKpi in index.html) ────
...
function computeKpi(metric, data, from, to) {
  ...
  switch (metric) {
    ...
    case 'cpd_total':
      return (data.cpdLog || []).filter(e => inPeriod(e.date, from, to)).reduce((a, e) => a + (e.hours || 0), 0);
    case 'cpd_avg': {
    ...
    case 'oto_coverage': {
      const ids = new Set(
        (data.onetoones || [])
          .filter(o => inPeriod(o.date, from, to))
    ...
    case 'disclosures_open':
      return (data.disclosures || []).filter(d => d.status !== 'resolved').length;
    default: return 0;
  }
}
...
const KPI_METRIC_LABELS = {
  headcount: 'Headcount',
  cpd_total: 'Total CPD Hours',
  cpd_avg: 'Avg CPD Hours/Person',
  review_coverage: 'Review Coverage',
  oto_coverage: '1:1 Coverage',
  training_complete: 'Training Completion',
  overdue_training: 'Overdue Assignments',
  skills_avg: 'Avg Skills Assessed',
  disclosures_open: 'Open Disclosures',
};

const KPI_METRIC_UNITS = {
...
function getPeriodRange(type, now) {
  const y = now.getFullYear();
  const m = now.getMonth(); // 0-based; we're on the 1st, so previous period = m-1
  ...
  if (type === 'monthly') {
    ...
    const label = new Date(py, pm - 1, 1).toLocaleDateString('en-NZ', { month: 'long', year: 'numeric' });
    return { from, to, label };
  }

  if (type === 'quarterly') {
    // Which quarter just ended?
    const q = Math.floor((m === 0 ? 11 : m - 1) / 3) + 1;
    const qy = m === 0 ? y - 1 : y;
    const sm = (q - 1) * 3 + 1;
    const em = sm + 2;
...
function buildHtml(period, type, rows) {
...
<div class="sub">${e(period.label)} · ${cap(type)} report · Auto-generated ${new Date().toLocaleDateString('en-NZ', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
...
${rows.map(r => {
    const unit = KPI_METRIC_UNITS[r.k.metric] || '';
    const label = KPI_METRIC_LABELS[r.k.metric] || r.k.metric;
    return `<tr><td><strong>${e(r.k.name)}</strong>${r.k.description ? `<br><span style="color:#888;font-size:.73rem">${e(r.k.description)}</span>` : ''}</td><td>${label}</td><td>${r.val}${unit === '%' ? '%' : ' ' + unit}</td><td>${r.k.targetDirection === 'lte' ? '≤' : '≥'} ${r.k.target}${unit === '%' ? '%' : ' ' + unit}</td><td class="${r.met ? 'met' : 'notmet'}">${r.met ? '✅ Met' : '❌ Not met'}</td></tr>`;
  }).join('')}
</tbody></table>
<div class="foot">McKenzie Growth Platform — This report was automatically generated.</div>
</body></html>`;
}
...
module.exports = async function (context, myTimer) {
  ...
  context.log(`scheduledReport fired: ${now.toISOString()}`);

  // Determine which frequencies to send
  const sendMonthly = day === 1;
  const sendQuarterly = day === 1 && [0, 3, 6, 9].includes(month); // Jan, Apr, Jul, Oct
  const sendAnnual = day === 1 && month === 0; // 1 January
  ...
  if (!recipients.length) {
    context.log('No recipients configured — skipping report send');
    return;
  }
  ...
  if (!smtpHost || !smtpUser || !smtpPass) {
    context.log.warn('SMTP not configured — cannot send scheduled report');
    return;
  }
  ...
  if (sendMonthly && settings.kpiAutoMonthly) frequenciesToSend.push('monthly');
  if (sendQuarterly && settings.kpiAutoQuarterly) frequenciesToSend.push('quarterly');
  if (sendAnnual && settings.kpiAutoAnnual) frequenciesToSend.push('annual');

  for (const freq of frequenciesToSend) {
    const period = getPeriodRange(freq, now);
    const activeKpis = kpis.filter(k => k.active);
    if (!activeKpis.length) { context.log(`No active KPIs for ${freq} report`); continue; }
    ...
    try {
      ...
      context.log(`Sent ${freq} KPI report to ${recipients.length} recipients`);
    } catch (err) {
      context.log.error(`Failed to send ${freq} report:`, err.message);
    }
  }
};
